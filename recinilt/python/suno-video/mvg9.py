...
def generate_prompts_with_claude(lyrics_segments, api_key, style_suffix="", mood="", log_callback=None):
    ...
    try:
        ...
        # DUYGU DURUMU ekleme (YENİ!)
        mood_instruction = ""
        if mood and mood.strip():
            mood_instruction = f"\n\nDUYGU DURUMU: Tüm görsellere '{mood}' duygu durumunu yansıt. Renk paleti, ışık ve atmosfer bu duyguya uygun olsun."
...
5. ASLA nudity, nsfw, explicit içerik ekleme!{style_instruction}{mood_instruction}
...
def load_stable_diffusion_model(model_id, device="cuda", dtype="float16", progress_callback=None):
    ...
    try:
        ...
        # fp16 variant'ı dene, yoksa normal yükle
        try:
            log("   🔄 fp16 variant deneniyor...")
            ...
            log("   ✓ fp16 variant yüklendi")
        except Exception as e:
            if "variant" in str(e).lower() or "fp16" in str(e).lower():
                log("   ⚠️ fp16 variant yok, normal yükleme yapılıyor...")
                pipe = StableDiffusionPipeline.from_pretrained(
                    model_id,
                    **load_params
                )
                log("   ✓ Normal variant yüklendi")
            else:
                raise e
...
class MusicVideoGeneratorGUI:
    ...
    def create_widgets(self):
        """GUI elemanlarını oluşturur - KAYDIRILABİLİR"""
        
        # Ana canvas ve scrollbar oluştur
        ...
        # Kaydırma için mouse wheel desteği
        ...
        # Artık scrollable_frame içinde widget'ları oluşturuyoruz
        ...
        # DUYGU DURUMU (YENİ!)
        ttk.Label(settings_frame, text="Duygu Durumu (Opsiyonel):").grid(row=2, column=0, sticky=tk.W)
        ttk.Entry(settings_frame, textvariable=self.mood_var, width=37).grid(row=2, column=1, sticky=(tk.W, tk.E), padx=5)
        mood_hint = ttk.Label(settings_frame, text="Örn: mutlu, hüzünlü, neşeli, karanlık...", 
                             font=('Arial', 8), foreground='gray')
        mood_hint.grid(row=3, column=1, sticky=tk.W, padx=5)
        
        # Ebat Seçimi
        ttk.Label(settings_frame, text="Ebat:").grid(row=4, column=0, sticky=tk.W, pady=(5,0))
        ...
        ratio_combo.grid(row=4, column=1, sticky=(tk.W, tk.E), padx=5, pady=(5,0))
        ratio_combo.bind('<<ComboboxSelected>>', self.on_ratio_change)
        
        # Özel boyut
        custom_frame = ttk.Frame(settings_frame)
        custom_frame.grid(row=5, column=1, sticky=(tk.W, tk.E), pady=5)
    ...
    def process_video(self):
        ...
        try:
            ...
            if mood_text:
                self.log(f"   Duygu Durumu: {mood_text}")
