...
class BinanceFuturesBot:
   ...
   def calculate_risk_table(self, initial_bet, multiplier, target_turn, stop_loss_percent, leverage):
       ...
       print(f"🛑 Zarar Durdur: {stop_loss_percent}% | ⚡ Likidite: {liquidation_percent:.2f}% → Kullanılan: {actual_stop_loss:.2f}%")
       print("-" * 70)
   ...
   def update_threshold(self, stop_loss_percent, leverage):
   ...
   def check_and_increase_initial_bet(self, stop_loss_percent, leverage):
       ...
       if self.stats['current_balance'] > self.threshold_amount:
           ...
           # Yeni eşiği hesapla
           self.update_threshold(stop_loss_percent, leverage)
   ...
   def handle_position_closed(self, symbol, direction, current_bet, leverage, side, profit_percent, stop_loss_percent):
       ...
       if limit_triggered:
           ...
           self.reset_position()
           self.check_and_increase_initial_bet(stop_loss_percent, leverage)
   ...
   def run_bot(self):
       ...
       if auto_increase in ['e', 'evet', 'y', 'yes']:
           ...
           # Risk tablosu göster ve ilk eşik hesaplama
           target_loss = self.calculate_risk_table(initial_amount, multiplier, self.target_turn, stop_loss_percent, leverage)
           self.update_threshold(stop_loss_percent, leverage)
           ...
           # Risk tablosu sadece gösterim için
           self.calculate_risk_table(initial_amount, multiplier, 10, stop_loss_percent, leverage)
           ...
           try:
               ...
               if not self.position_opened:
                   ...
                   if self.check_limit_order(symbol, direction):
                       ...
                       # İlk bahis kontrolü yap ve başa dön
                       self.check_and_increase_initial_bet(stop_loss_percent, leverage)
                       current_bet = self.current_initial_bet
