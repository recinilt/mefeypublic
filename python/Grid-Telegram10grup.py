...
async def main():
    ...
    try:
        ...
        async def send_question(user, chat_id, event=None):
            """Sıradaki soruyu gruba, kullanıcıyı alıntılayarak gönderir."""
            if user not in user_data:
                user_data[user] = {"answers": [], "question_index": 0, "processing": False}
            ...
            if question_index < len(soracaklarim):
                if event:
                    await event.reply(soracaklarim[question_index])
                else:
                    await telegram_client.send_message(chat_id, soracaklarim[question_index], reply_to=user)
                ...
                try:
                    ...
                    if event:
                        ...
                        await telegram_client.send_message(chat_id, result, reply_to=user)
                except Exception as e:
                    if event:
                        await event.reply(f"Hata oluştu: {e}")
                    else:
                        await telegram_client.send_message(chat_id, f"Hata oluştu: {e}", reply_to=user)
                finally:
