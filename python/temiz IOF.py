...
def AnaFonkKA(raw_text):
    ...
    if result[0][0]>1:
        ...
        kadakilonglar=[]
        for coin in result:
            if binle(coin[0]) in mysymbols3 and coin[2]<2 and coin[3]>1.03 and coin[5]==True and coin[6]<10:
...
def AnaFonkIOF(raw_text):
    ...
    if check_arrowsIO(mytextio[0]):
        ...
        for entry in parsed_data:
            if binle(entry[0]) in mysymbols3 and entry[1]>1 and entry[2]>0.5 and entry[3]<5 and entry[4][0] and entry[4][1] and entry[4][2] and entry[4][3]:
    ...
    if not check_arrowsIO(mytextio[0]): #len(combined_list2) > 0: 
        shortAc=[]
        for entry in parsed_data:
            if binle(entry[0]) in mysymbols3 and entry[1]<1 and entry[2]>0.5 and entry[3]>0.5 and not entry[4][0] and not entry[4][1] and not entry[4][2] and not entry[4][3] and not entry[4][4]:
