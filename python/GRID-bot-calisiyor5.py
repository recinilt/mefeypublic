...
def split_levels(levels, close):
    if True:#close>start and close<end:
        ...
        if oncekialinacaklevelsonprice==1000000:
            if alinacaklar:
        ...
        if close> oncekialinacaklevelsonprice and close<oncekisatilacaklevelilkprice:
...
for i in range(minmiktarondalik):
    carpim=carpim*10
mymyq=math.ceil(mymyq * carpim) / carpim #yukarıya doğru hep aynı quantity de alınacak.
...
myassetsasagi=5.5*len(alinacaklar)
ilkbakiye=myassetsasagi + (mymyq*close)*len(satilacaklar)
        ...
        if 'close' in data.columns:
            ...
            for index, value in tqdm(enumerate(data['close']), total=len(data['close']), desc="Processing close values"):
                ...
                if value>start and value<end:
                    try:
                        new_alinacaklar, new_satilacaklar = split_levels(levels, value)
                    except:
                    ...
                    if fazlalikalinacaklar:
                        #print("alınacaklar: ", fazlalikalinacaklar)
                        if value*mymyq>5.5:
                            gridprofit+= value*mymyq*(geometric_percentage-comission)*0.01*len(fazlalikalinacaklar)#len(fazlalikalinacaklar)*(geometric_percentage-comission)*0.01*5.5*(5.5/value)
                        else:
                            gridprofit+= 5.5*(geometric_percentage-comission)*0.01*len(fazlalikalinacaklar)#len(fazlalikalinacaklar)*(geometric_percentage-comission)*0.01*5.5*(5.5/value)
                ...
                #old_satilacaklar=new_satilacaklar[:]
            myqs=0.0
            for q in old_satilacaklar: #iptal edildi.
                myq=5.5/q
                carpim=1
                for i in range(minmiktarondalik):
                    carpim=carpim*10
            ...
            print("myqs",mysatilacakq)
            alinacaklartoplamusdt=0.0
            for alinacakusdt in old_alinacaklar:
                alinacaklartoplamusdt+=alinacakusdt*mymyq
