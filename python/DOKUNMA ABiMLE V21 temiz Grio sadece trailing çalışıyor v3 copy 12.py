...
def AnaFonkIoGroup(raw_text):
    ...
    if io1d[-1]>48: #almakkosulu():
        if io1d[-1]>48 and apalayimmi and parsed_data["group_data"]["12h"]>49.7 and parsed_data["group_data"]["1d"]>50 and parsed_data["group_data"]["short_term_buy_power"]>=1.1 and parsed_data["group_data"]["market_volume_share"]>=1:
            print("grup iyi durumda grio")
            #print("merhaba")
            for coin in parsed_data["coin_data"]:
                if (coin["nakit"] * 0.01 * parsed_data["group_data"]["market_volume_share"]>=0.05) and coin["trend_up"][3] and coin["trend_up"][4] and coin["mts"]>1 and coin["mts"]<3:
