...
myquantityhazir=myquantity(islemcoin)
############################ kar zarar durumu
...
def calculate_hlc3_vwap(data, lookback_period=14):
    """Verilen kline verilerinden HLC3 ve periyodik VWAP hesapla."""
    if len(data) < lookback_period:
        raise ValueError("Yeterli veri yok. Lookback periyodundan daha fazla veri gereklidir.")
    ...
    for entry in data[-lookback_period:]:
        high = float(entry[2])
        low = float(entry[3])
        close = float(entry[4])
        volume = float(entry[5])
        myclose.append(close)
        hlc3 = (high + low + close) / 3
        tp_volume = hlc3 * volume

        hlc3_values.append(hlc3)
        tp_volumes.append(tp_volume)
        volumes.append(volume)

    sum_tp_volume = sum(tp_volumes)
    sum_volume = sum(volumes)

    vwap = sum_tp_volume / sum_volume if sum_volume > 0 else 0
    latest_hlc3 = hlc3_values[-1]

    return latest_hlc3, vwap, myclose[-1]

ilkmi = True
longmu = 3  # 1: long, 2: short

def main():
    global ilkmi
    global longmu
    symbol = islemcoin #"SUSHIUSDT"  # İşlem çifti (örneğin, BTCUSDT)
    interval = "1m"  # 1 dakikalık zaman dilimi
    closes=[]
    while True:
        try:
            # Binance API'den kline verilerini al
            data = get_binance_klines(symbol, interval, limit=30)  # Daha fazla veri alın, lookback için gerekli olabilir

            # HLC3 ve VWAP hesapla
            hlc3, vwap, close = calculate_hlc3_vwap(data, lookback_period=14)
            closes.append(close)
            # HLC3, VWAP'ın üstünde mi?
            result = hlc3 > vwap
            print(f"HLC3: {hlc3}, VWAP: {vwap}, Result: {result}")
            if result == True and (longmu == 2 or longmu == 3):
                if ilkmi == True:
                    threaded_buy_position(symbol, kacxkaldirac, harcanancost, "mylonglarGenel", islemyapilacakkisi)
                    ilkmi = False
                    longmu = 1
                    closes.clear()
                else:
                    threaded_buy_position(symbol, kacxkaldirac, harcanancost * 2, "mylonglarGenel", islemyapilacakkisi)
                    longmu = 1
                    closes.clear()
            elif result == False and (longmu == 1 or longmu == 3):
                if ilkmi == True:
                    threaded_sell_position(symbol, kacxkaldirac, harcanancost, "myshortlarGenel", islemyapilacakkisi)
                    ilkmi = False
                    longmu = 2
                    closes.clear()
                else:
                    threaded_sell_position(symbol, kacxkaldirac, harcanancost * 2, "myshortlarGenel", islemyapilacakkisi)
                    longmu = 2
                    closes.clear()
            
            if longmu==1 and max(closes)*0.9985>closes[-1]:
                threaded_close_position(symbol,mylonglarGenel,islemyapilacakkisi)
                ilkmi=True
            if longmu==2 and min(closes)*1.0015<closes[-1]:
                threaded_close_position(symbol,myshortlarGenel,islemyapilacakkisi)
                ilkmi=True
