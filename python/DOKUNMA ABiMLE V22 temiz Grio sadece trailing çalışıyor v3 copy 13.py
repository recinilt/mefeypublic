...
def almakkosulu():
    myresult = (io1d[-1]>48.9 and apalayimmi and (cift_ema_sinyal(io1d)[0]) and (not son_bes_esit_mi(io1d))) or (io1d[-1]>49.6 and apalayimmi)
    return myresult
...
def AnaFonkIOF2(raw_text):
    ...
    if almakkosulu():# io1d[-1]>48.9 and apalayimmi:
        for mylist in parsed_data:
            if binle(mylist[0]) in mysymbols3 and mylist[1]>2 and mylist[2]>0.4 and mylist[3]>1 and mylist[3]<1.7 and mylist[4][0] and mylist[4][1] and mylist[4][2] :
...
def AnaFonkIoGroup(raw_text):
    ...
    """
    longacilacaklar=[]
    if io1d[-1]>48: #almakkosulu():
        if io1d[-1]>48 and apalayimmi and parsed_data["group_data"]["12h"]>49.7 and parsed_data["group_data"]["1d"]>50 and parsed_data["group_data"]["short_term_buy_power"]>=1 and parsed_data["group_data"]["market_volume_share"]>=1:
            print("grup iyi durumda grio")
            #print("merhaba")
            for coin in parsed_data["coin_data"]:
                if (coin["nakit"] * 0.01 * parsed_data["group_data"]["market_volume_share"]>=0.05) and coin["trend_up"][3] and coin["trend_up"][4] and coin["mts"]>1 and coin["mts"]<2:
