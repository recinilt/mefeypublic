...
def save_to_m3u(working_streams, output_file="working_channels.m3u"):
    """
    Çalışan kanalları bir M3U dosyasına kaydeder.
    """
    with open(output_file, "w", encoding="utf-8") as file:
        file.write("#EXTM3U\n")
        for channel_name, stream_url in working_streams:
            file.write(f"#EXTINF:-1, {channel_name}\n")
            file.write(f"{stream_url}\n")
    print(f"Çalışan kanallar {output_file} dosyasına kaydedildi.")
...
# Çalışan kanalları tutmak için liste
working_channels = []
...
for page_url in page_urls:
    ...
    if stream_urls:
        ...
        if check_url(first_stream_url):
            ...
            # Sayfa URL'sini kanal adı olarak kaydediyoruz
            channel_name = page_url.split("/")[-1].capitalize()
            working_channels.append((channel_name, first_stream_url))
        else:
            print(f"Çalışmıyor: {first_stream_url}")

# Çalışan kanalları bir M3U dosyasına kaydet
save_to_m3u(working_channels)
