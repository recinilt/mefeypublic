...
symbols=coins[:100]
total=[]
def longlarıtara(longacmaoran,acikkalmaorani, shortacmaorani, shortacikkalmaorani):
    acikkalacaklar=[]
    longacilacaklar=[]
    shortacilacaklar=[]
    shortacikkalacaklar=[]
    for mycoin in symbols:
        ...
        try:
            ...
            print("hata: ", e)
        total.append(result)
        if result>longacmaoran:
            longacilacaklar.append([mycoin,result])
            print("LONG>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>: ", mycoin, result)
        if result>acikkalmaorani:
            acikkalacaklar.append([mycoin,result])
            #print("LONG>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>: ", mycoin, result)
        if result<shortacmaorani:
            shortacilacaklar.append([mycoin,result])
            print("SHORT------------------------------------------: ", mycoin, result)
        if result<shortacikkalmaorani:
            shortacikkalacaklar.append([mycoin,result])

    if longacilacaklar:
        longacilacaklar.sort(key=lambda x: x[1], reverse=True)
    return [longacilacaklar, acikkalacaklar,shortacilacaklar,shortacikkalacaklar]
...
def calculate_ema(prices, span):
    ...
    for i, price in enumerate(prices):
        if i == 0:
            ema.append(price)  # İlk fiyat EMA olarak alınır
        else:
...
def analyze_market():
    ...
    try:
        ...
        # EMA'ları hesaplayın
        short_ema = calculate_ema(close_prices, 27)
        long_ema = calculate_ema(close_prices, 147)
...
def get_vwap(symbol="BTCUSDT", interval="1m", lookback_period=14):
    # Binance API endpoint
    base_url = "https://api.binance.com/api/v3/klines"
    params = {
        "symbol": symbol,
        "interval": interval,
        "limit": lookback_period  # Geriye dönük 14 mum çubuğu al
    }
    ...
    if response.status_code == 200:
        ...
        if len(data) > 0:
            ...
            total_tp_volume = 0  # Typical Price * Volume toplamı
            total_volume = 0     # Toplam hacim

            # Her mum çubuğu için hesaplama
            for kline in data:
                high = float(kline[2])  # En yüksek fiyat
                low = float(kline[3])   # En düşük fiyat
                close = float(kline[4]) # Kapanış fiyatı
                volume = float(kline[5])# İşlem hacmi (miktar)

                # Typical Price (TP) hesaplama
                typical_price = (high + low + close) / 3

                # TP * Volume hesapla ve toplama ekle
            ...
            if total_volume > 0:
                ...
                return [vwap,float(data[-1][4])]
            else:
                return "Volume is zero, cannot calculate VWAP."
        else:
...
# Kullanım
vwap_value = get_vwap()
print(f"VWAP for the last 14 periods: {vwap_value}")
...
while True:
    ...
    ###########
    if True:
        tarananlar=longlarıtara(3,2,0.8,0.9)
        btcdurumu = analyze_market()
        taranansemboller= [item[0] for item in tarananlar[0]]
        acikkalacaklar=[item[0] for item in tarananlar[1]]
        shortacilacaklar=[item[0] for item in tarananlar[2]]
        print("Long açılacakalr: ", taranansemboller)
        print("Short açılacaklar: ",shortacilacaklar)
        shortacikkalacaklar=[item[0] for item in tarananlar[3]]
        #total = sum(item[1] for item in tarananlar)
        mytotal=sum(total)
        tumoran=mytotal/len(total)
        tumoranlist.append(tumoran)
        print("tümoran: ",tumoran)
        print("BTC Durumu: ", btcdurumu)
        vwap_value = get_vwap() #0:vwap, 1:btc price
        if False:#tumoran>1.1 and cift_ema_sinyal()[1]:
            if mylonglarGenel:
                for coin in mylonglarGenel:
                    if not coin in acikkalacaklar:
                        threaded_close_position(coin,"mylonglarGenel",islemyapilacakkisi)
                        #winsound.PlaySound(satsesi, winsound.SND_FILENAME)
                        
        if False:#(tumoran<tumorankactankucukoluncasatsin and cift_ema_sinyal()[1]) or tumoran<tumorankactankucukoluncasatsin or btcdurumu==False:
        ...
        if False:#tumoran>tumorankactanbuyukoluncaalsin and btcdurumu:
            for coinac in taranansemboller:
                if not coinac in mylonglarGenel:
                    threaded_buy_position(coinac,6,1.8,"mylonglarGenel",islemyapilacakkisi)
                    #winsound.PlaySound(alsesi, winsound.SND_FILENAME)
        
        if vwap_value[0]*0.9985>vwap_value[1]: #btcdurumu==False: 
            for shortac in shortacilacaklar:
                if not shortac in myshortlarGenel:
                    threaded_sell_position(shortac,6,1.8,"myshortlarGenel",islemyapilacakkisi)
                    #winsound.PlaySound(alsesi, winsound.SND_FILENAME)
        if vwap_value[0]*1.0015<vwap_value[1] and btcdurumu==True:
            if myshortlarGenel:
        ...
        if myshortlarGenel:
            for coin in myshortlarGenel:
                if not coin in shortacikkalacaklar:
                    threaded_close_position(coin,"myshortlarGenel",islemyapilacakkisi)
                    #winsound.PlaySound(satsesi, winsound.SND_FILENAME)
