...
def ilkbakiyehesapla():
    ...
    gerekliustusdt=(math.ceil((close*q1)*carpim)/carpim)*len(old_satilacaklar)
...
def process_close_values(data):
    ...
    try:
        ...
        if len(data) > 0:
            ...
            suankibakiye=sonbakiyehesapla(sonkapanis,old_alinacaklar,old_satilacaklar)
            kazancorani = (suankibakiye + gridprofit) / ilkbakiye
            girilenboluminbakiye=round((girilenilkbakiye/ilkbakiye),2)
            ...
            print("Şuanki gridden gelen hariç bakiye: ", round(suankibakiye*girilenboluminbakiye , 2))
            print("Şuanki grid dahil bakiye / ilk bakiye= ", round(kazancorani, 2))
            print("Coin ilk fiyatı: ",close)
