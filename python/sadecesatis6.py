...
def AnaFonkSatis():
    ...
    if True:
        if len(mydusenler[0])>3:
            ...
            if not len(myshortlarGenel)==0:
                ...
                while True:
                    kapatmismi=input("Tüm pozisyonları kapattın ise küçük harfle y yaz: ")
    ...
    #Alış:
    if False:
        if len(mydusenler[2])>3:
            for coin in mydusenler[3]:

                if not coin in mylonglarGenel:
                    buy_position(coin, 20, 1, "mylonglarGenel")
        elif len(mydusenler[2])<3:
