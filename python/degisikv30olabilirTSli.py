        ...
        for pos in positions:
            ...
            if io1d[-1]<49:
                if groupacilanlar:
                    if not any(item[1] == binle(pos["Symbol"]) for item in groupacilanlar):
                        if myshortlarGenel:
                            if not any(item == binle(pos["Symbol"]) for item in myshortlarGenel):
                                io49unaltinda.append(binle(pos["Symbol"]))   
                        else:
                            io49unaltinda.append(binle(pos["Symbol"])) 
                elif myshortlarGenel:
                    if not any(item == binle(pos["Symbol"]) for item in myshortlarGenel):
                        io49unaltinda.append(binle(pos["Symbol"]))    
                else:
                    io49unaltinda.append(binle(pos["Symbol"]))
...
########################### ts
def extract_usdt_coins_ts(text, start_keyword="Güçlü Yükseliş Yapanlar Yükseliş Skoru", end_keyword="Güçlü Yükseliş Trendinde Olup En Ucuz Olanlar Yükseliş Skoru"):
...
def AnaFonkTs(raw_text):
    global myshortlarGenel
    global apkisaalayimmi
    global apuzunalayimmi
    global apalayimmi
    global apalayimmi
    ...
    global groupacilanlar
    calissinmi= False
    parsed_data=extract_usdt_coins_ts(raw_text)
    print(f"ts: {parsed_data}")
    shortacilacaklar=[]
    if io1d[-1]<49:
        for coin in shortacilacaklar:
            if binle(coin) in myshortlarGenel:
                print(f"{coin} zaten vardı ts")
            elif binle(coin) in yasaklilist:
                print(f"Açılamayan coin: {coin}")
            else:
                #mylonglarGenel.append(coin)
                sell_position(binle(coin), myleverage, get_my_cost_hazir * 1, "myshortlarGenel")
                print(f"ts ile, {coin} short açıldı")
    if io1d[-1]>49:
        print(f"ts kapatılacaklar: {myshortlarGenel} ")
        for c in myshortlarGenel:
            close_position(c,"myshortlarGenel")
...
async def main():
    ...
    async def handler(event):
        ...
        elif event.raw_text.startswith("Trend Skoru Raporu Yorumları"): #ts
            
            while True:
                if calissinmi:
                    AnaFonkTs(event.raw_text)
                    print("ts")
                    break
                else:
                    await asyncio.sleep(5)
    ...
    while True:
        if True:
            ...
            while True:
                if calissinmi:
                    await telegram_client.send_message(target_user, "ts" if io1d[-1]<49 else "io") #mysent[rastgele_komut()] )#'marketanaliz')
                    await asyncio.sleep(rastgele_sayi(35,100))
                    break
                else:
                    await asyncio.sleep(2)
