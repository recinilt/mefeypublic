...
async def main():
    ...
    try:
        ...
        async def handle_response(event):
            """Grup mesajlarını işler ve USDT'li mesajları tetikler."""
            global user_data
            user = event.sender_id
            message = event.raw_text.strip()

            # Eğer kullanıcı özel bir süreç başlatmamışsa, başlat
            ...
            # USDT'li mesaj tetikleme
            if "başlat" in message.lower():
                if not user_data[user]["processing"]:
                    await send_question(user, event.chat_id, event)
                else:
                    await event.reply("Cevabınız işlenirken lütfen bekleyiniz.")
                return

            # Normal yanıt işleme
            if user_data[user]["processing"]:
                await event.reply("Cevabınız üretilirken lütfen bekleyiniz.")
                return
        ...
        async def send_question(user, chat_id, event=None):
            """Sıradaki soruyu gruba, mesaj alıntılayarak gönderir."""
            if user not in user_data:
                user_data[user] = {"answers": [], "question_index": 0, "processing": False}
            ...
            if question_index < len(soracaklarim):
                ...
                if event:
                    await telegram_client.send_message(chat_id, question, reply_to=event.message.id)
                else:
                    await telegram_client.send_message(chat_id, question)
            elif not user_data[user]["processing"]:
                # Sorular tamamlandı, işlem başlıyor
                user_data[user]["processing"] = True
                try:
                    result = await asyncio.to_thread(myanafonk, user_data[user]["answers"])
                    if event:
                        await telegram_client.send_message(chat_id, result, reply_to=event.message.id)
                    else:
                        await telegram_client.send_message(chat_id, result)
                except Exception as e:
                    if event:
                        await telegram_client.send_message(chat_id, f"Hata oluştu: {e}", reply_to=event.message.id)
                    else:
                        await telegram_client.send_message(chat_id, f"Hata oluştu: {e}")
                finally:
                    # Kullanıcı için veriler sıfırlanıyor
                    user_data[user] = {"answers": [], "question_index": 0, "processing": False}
