...
#patterler
smainterval = Client.KLINE_INTERVAL_15MINUTE
smaperiod = 30
...
############# Basit Hareketli Ortalama Fonksiyonu
#symbol = 'BTCUSDT'
...
def fetch_data(coin):
    klines = binanceclient.get_klines(symbol=coin, interval=smainterval, limit=smaperiod)
    closes = [float(kline[4]) for kline in klines]
    return closes

def trade(coin):
    closes = fetch_data(coin)
    sma = get_sma(closes, smaperiod)
    last_close = closes[-1]

    if last_close > sma:
        print("SMA'nın üzerinde: Al")
        return True
        # Buraya alım emrini ekleyin
    else:
        print("SMA'nın altında: Sat")
        return False
        # Buraya satım emrini ekleyin
...
def buy15m_position(symbol, leverage, amount):
    try:
        if trade(symbol):
            binanceclient.futures_change_leverage(symbol=symbol, leverage=leverage)
            #binanceclient.futures_change_margin_type(symbol=symbol, marginType=ISOLATED)
...
def sell15m_position(symbol, leverage, amount):
    try:
        if not trade(symbol):
            binanceclient.futures_change_leverage(symbol=symbol, leverage=leverage)
            #binanceclient.futures_change_margin_type(symbol=symbol, marginType=ISOLATED)
...
def AnaFonkCi15m(text):
    #if 1: #not check_arrowsIO(mytextio[0]):
    if not "USDT" in text:   
        text = myci15mrawtext[0]
    else:
        myci15mrawtext.clear()
        myci15mrawtext.append(text)
    patternCiid15m = r'\b(\w+USDT)\s+(\d+,\d+)\s+(\d+,\d+)\s+(\d+)\s+(\d+,\d+)'
    matchesCiid15m = re.findall(patternCiid15m, text)
    resultCiid15m = [[match[0], float(match[1].replace(',', '.')), float(match[2].replace(',', '.')), float(match[3]), float(match[4].replace(',', '.'))] for match in matchesCiid15m]
    longAc=[]
    shortAc=[]
    for c in resultCiid15m:
        if binle(c[0]) in mysymbols3 and acabilirmiyim(binle(c[0])):
            if ((c[1]-c[2]>0.02) and c[1]>1 and c[2]<1) or (c[1]-c[2]>0.06):
                if c[4]<6:
                    longAc.append(binle(c[0]))
            if ((c[2]-c[1]>0.02) and c[1]<1 and c[2]>1) or (c[2]-c[1]>0.06):
                if c[4]>0.7:
                    shortAc.append(binle(c[0]))
    
    if 1: #check_arrowsIO(mytextio[0])  and io1d[len(io1d)-1]>altustsinir[1]:
        for coin in longAc:
            if coin in mylonglarCi:
                print(f"{coin} zaten vardı")
            else:
                mylonglarCi.append(coin)
                buy15m_position(coin, myleverage, mycost)
                print(f"{coin} long açıldı")
                #await telegram_client.send_message(alert_user, f"{coin}'a LONG posizyon açıldı. büyüklüğü: {myleverage}x kaldıraçlı, {mycost} USDT harcamalı, yani {myleverage * mycost} dolar büyüklüğünde.")
        for coin in mylonglarCi:
            if coin in longAc:
    ...
    if True: #not check_arrowsIO(mytextio[0]) and io1d[len(io1d)-1]<altustsinir[0]:
        for coin in shortAc:
            if coin in myshortlarCi:
                print(f"{coin} zaten vardı")
            else:
                myshortlarCi.append(coin)
                sell15m_position(coin, myleverage, mycost)
                print(f"{coin} short açıldı")
                #await telegram_client.send_message(alert_user, f"{coin}'a SHORT posizyon açıldı. büyüklüğü: {myleverage}x kaldıraçlı, {mycost} USDT harcamalı, yani {myleverage * mycost} dolar büyüklüğünde.")
        for coin in myshortlarCi:
            if coin in shortAc:
...
async def main():
    ...
    async def handler(event):
        ...
        if event.raw_text.startswith("Korelasyon Şiddeti Raporu (15m)"): #ci i d 15m
            AnaFonkCi15m(event.raw_text)
