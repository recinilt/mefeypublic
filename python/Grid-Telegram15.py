...
telegram_api_id = '20798991'
telegram_api_hash = '4b8f8f2fc7d04c5bf42ae4ab57881d3f'
phone_number = '+905421582059'
target_users = ['reccirik',"mefey18","OrhanElmas","nebe18","Mymirzabey57","akman03","Samiay31"]
chat_id="https://t.me/gridbotkartesti"
#################
...
end_date="2024-07-27"
#################
soracaklarim=["Bu bir telegram botu mesajıdır. Grid bactestine baştan başlıyoruz. Grid botu, verdiğiniz tarih aralıklarında spot grid bot yapsanız, kaç dolar elde ederdiniz, bunu öğreneceksiniz. Lütfen, parantez içlerindeki örnek formatında giriniz. istediğiniz zaman başlat yazarak soruları baştan başlatabilirsiniz. \nCoin Çiftini Giriniz (Örn:     ONEUSDT      ): ", "Alt seviyeyi giriniz (Örn: 0.02): ","Üst seviyeyi giriniz (Örn: 0.6): ","Grid sayısını giriniz (alt üst sınır arası kaça bölünsün?) (Binance limit=max 200) (Örn: 200): ","Komisyon yüzdesi (Örn: 0.1 (yani binde bir) (sadece sayı giriniz, yüzde işareti girmeyiniz)): ","Başlangıç sermayesi kaç USDT olsun (Örn. 1000): ","Zaman dilimi ne olsun? (1 dakikalıklarda kontrol etmek için 1m yazınız) (Örn.: 1m, 5m, 1h, 1d): ","Botun çalışma başlangıç tarihini yıl tire ay tire gün şeklinde yazınız (Örn. 2024-01-17): ","Botun çalışma bitiş tarihini yıl tire ay tire gün şeklinde yazınız (Örn. 2024-12-27): \n(bu soruyu cevapladıktan sonra lütfen işlemin bitmesini bekleyiniz, cevabınız gelecek. 365 günlük veri işlenmesi yaklaşık 20 dakika sürer...)"]
...
def vericekis(veriinterval, start_date, end_date):
    ...
    Args:
        veriinterval (str): Zaman dilimi (örn. "1m", "5m", "1h", "1d").
        start_date (str): Başlangıç tarihi ("YYYY-MM-DD").
        end_date (str): Bitiş tarihi ("YYYY-MM-DD").
...
def process_close_values(data):
    ...
    try:
        ...
        if len(data) > 0:
            ...
            Grid'den aylık yüzde kazanç: %{round(30*((gridprofit / yavilkbakiyeiste) * 100)/calculate_days_between(start_date,end_date) ,2)}
            Grid'den yıllık yüzde kazanç: %{12*round(30*((gridprofit / yavilkbakiyeiste) * 100)/calculate_days_between(start_date,end_date) ,2)}
            ...
            file_name = f"{symbol}-{calculate_days_between(start_date,end_date)}gun-gridKarYuzde{round(round((gridprofit / yavilkbakiyeiste), 2) * 100,2)}-bakiyeX{round(kazancorani, 2)}-aralikYuzde{geometric_percentage}-{get_current_datetime_text()}"
...
async def main():
    ...
    telegram_client = TelegramClient('group_session_name', telegram_api_id, telegram_api_hash, 
                                      flood_sleep_threshold=60, connection_retries=5, request_retries=5)
    ...
    try:
        ...
        async def handle_response(event):
            """Grup mesajlarını işler ve kullanıcı komutlarına göre hareket eder."""
            global user_data
            user = event.sender_id
            message = event.raw_text.strip()

            # Eğer kullanıcı henüz süreç başlatmamışsa ve "başlat" yazmamışsa, işlem yapılmaz
            if user not in user_data:
                if message.lower() == "başlat":
                    user_data[user] = {"answers": [], "question_index": 0, "processing": False}
                    await event.reply("Süreç başlatıldı. İlk soruyla başlıyoruz.")
                    await send_question(user, event.chat_id)
                return

            # Süreç sıfırlama komutu
            if message.lower() == "başlat":
                user_data[user] = {"answers": [], "question_index": 0, "processing": False}
                await event.reply("Süreç baştan başlatıldı.")
                await send_question(user, event.chat_id)
                return

            # Kullanıcı işlem halindeyken mesaj göndermişse, beklemesi istenir
            if user_data[user]["processing"]:
                await event.reply("İşlem devam ediyor. Lütfen bitmesini bekleyin.")
                return

            # Kullanıcı cevabını kaydet ve bir sonraki soruyu sor
            user_data[user]["answers"].append(message)
            user_data[user]["question_index"] += 1
            await send_question(user, event.chat_id)


        async def send_question(user, chat_id):
            """Sıradaki soruyu gönderir veya işlemi tamamlar."""
            global user_data
            ...
            if question_index < len(soracaklarim):
                question = soracaklarim[question_index]
                await telegram_client.send_message(chat_id, question)
            elif not user_data[user]["processing"]:
                # Sorular tamamlandı, işlem başlatılıyor
                user_data[user]["processing"] = True
                try:
                    result = await asyncio.to_thread(myanafonk, user_data[user]["answers"])
                    await telegram_client.send_message(chat_id, result)
                except Exception as e:
                    await telegram_client.send_message(chat_id, f"Hata oluştu: {e}")
                finally:
                    # İşlem tamamlandı, kullanıcı verileri kesin olarak sıfırlanıyor
                    user_data.pop(user, None)  # Kullanıcı verisini kaldır
