...
old_satilacaklar=[]
#################
soracaklarim=["Grid bactestine baştan başlıyoruz. Coin Çiftini Giriniz (Örn: BTCUSDT): ", "Alt seviyeyi giriniz (Örn: 0.02): ","Üst seviyeyi giriniz (Örn: 0.6): ","Grid sayısını giriniz (Binance limit=max 200) (Örn: 200): ","Komisyon (Örn: %0.1 (yani binde bir)): %","Başlangıç sermayesi kaç USDT olsun (Örn. 1000): ","Enter the interval (e.g., 1m, 5m, 1h, 1d): ","Enter the number of days back to fetch (e.g. 3): \n(bu soruyu cevapladıktan sonra lütfen işlemin bitmesini bekleyiniz, cevabınız gelecek. 365 günlük veri işlenmesi yaklaşık 20 dakika sürer...)"]
...
def process_close_values(data):
    ...
    try:
        ...
        if len(data) > 0:
            ...
            file_name = f"{symbol}-{veri_days_back}gun-gridYuzde{round((gridprofit / yavilkbakiyeiste), 2) * 100}-bakiyeYuzde{round(kazancorani, 2)}-aralikYuzde{geometric_percentage}-{get_current_datetime_text()}"
...
async def main():
    ...
    async def send_question(user):
        '''Sıradaki soruyu kullanıcıya gönderir.'''
        if user not in user_data:
            user_data[user] = {"answers": [], "question_index": 0}
        ...
        if question_index < len(soracaklarim):
            ...
            # Tüm sorular soruldu, yanıtları işle
            result = myanafonk(user_data[user]["answers"])
            await telegram_client.send_message(user, result)
            # Başlangıç değerlerine dön
            user_data[user] = {"answers": [], "question_index": 0}
            await send_question(user)
