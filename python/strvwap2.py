...
client.futures_change_leverage(symbol="AAVEUSDT", leverage=75)  # Set leverage to 100x

# Parameters
symbol = "AAVEUSDT"  # Futures trading pair
interval = "1m"  # 1-minute interval
vwap_offset = 0.0015  # 0.15%
take_profit_perc = 0.0050  # 1.2%
stop_loss_perc = 0.0015  # 0.13%
quantity = 0.2  # Adjust based on balance and risk
...
while True:
    try:
        ...
        if not has_open_position:
            ...
            long_condition = close_price > long_entry_level
            if long_condition:
            ...
            short_condition = close_price < short_entry_level
            if short_condition:
                take_profit_price = close_price * (1 - take_profit_perc)
                stop_loss_price = close_price * (1 + stop_loss_perc)
