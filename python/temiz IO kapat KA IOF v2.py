...
def AnaFonkKA(raw_text):
    ...
    #print(result)
    if result[0][0]>-1 and is_above_last_period_average(io1d[-1],io1d,smaperiod): # and not io15m1h4hdusuktemi():
        myFKAlist=[]
        kadakilonglar=[]
        for coin in result:
            print(coin[0], coin[2],coin[3],coin[5],coin[6])
            if binle(coin[0]) in mysymbols3 and coin[2]<2 and coin[3]>1.01 and coin[5]==True and coin[6]<10 and acabilirmiyim(binle(coin[0])):
...
def AnaFonkCi(text):
    ...
    for c in resultCiid5m:
        if binle(c[0]) in mysymbols3 and acabilirmiyim(binle(c[0])):
            if ((c[1]-c[2]>0.02) and c[1]>1 and c[2]<1) or (c[1]-c[2]>0.07):
                if c[4]<6:
                    longAc.append(binle(c[0]))
            if ((c[2]-c[1]>0.02) and c[1]<1 and c[2]>1)or (c[2]-c[1]>0.07):
                if c[4]>0.7:
    ...
    if not is_above_last_period_average(io1d[-1],io1d,smaperiod): #not is_above_last_period_average(io1d[-1],io1d,smaperiod): #not check_arrowsIO(mytextio[0]) and io1d[len(io1d)-1]<altustsinir[0]:
        for coin in shortAc:
