...
target_users = ['reccirik',"mefey18","OrhanElmas","nebe18","Mymirzabey57","akman03"]
#################
...
def split_levels(levels, myclose,high=None,low=None):
    if True:#close>start and close<end:
        ...
        if oncekisatilacaklevelilkprice==1000000:
            ...
            if satilacaklar:
                oncekisatilacaklevelilkprice=satilacaklar[0]
            if alinacaklar:
                oncekialinacaklevelsonprice=alinacaklar[-1]
        ...
        if low is not None and high is not None:
            ...
            if low> oncekialinacaklevelsonprice and high<oncekisatilacaklevelilkprice:
                ...
                if satilacaklar:
                    oncekisatilacaklevelilkprice=satilacaklar[0]
                if alinacaklar:
                    oncekialinacaklevelsonprice=alinacaklar[-1] #BURADA TAKILIYOR
...
def process_close_values(data):
    ...
    try:
        ...
        if len(data) > 0:
            ...
            Şuanki grid dahil bakiye bölü ilk bakiye: {round(kazancorani, 2)} X
            ...
            file_name = f"{symbol}-{veri_days_back}gun-gridYuzde{round((gridprofit / yavilkbakiyeiste), 2) * 100}-bakiyeX{round(kazancorani, 2)}-aralikYuzde{geometric_percentage}-{get_current_datetime_text()}"
