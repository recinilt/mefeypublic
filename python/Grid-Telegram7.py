...
def process_close_values(data):
    ...
    try:
        ...
        if len(data) > 0:
            ...
            file_name = f"{symbol}-{calculate_days_between(start_date,end_date)}gun-gridKarYuzde{round((gridprofit / yavilkbakiyeiste), 2) * 100}-bakiyeX{round(kazancorani, 2)}-aralikYuzde{geometric_percentage}-{get_current_datetime_text()}"
    ...
    try:
        ...
        async def send_question(user):
            ...
            if user not in target_users:
                return  # Kullanıcı listede değilse işlem yapma
            ...
            if question_index < len(soracaklarim):
                ...
                # Tüm sorular tamamlandı, yanıtları işle
                ...
                try:
                    ...
                    # Kullanıcıyı sıfırla ve soruları yeniden başlat
                    user_data[user] = {"answers": [], "question_index": 0, "processing": False}
        ...
        async def handle_admin_commands(event):
            ...
            if message.startswith("yenikayit-"):
                new_user = message.split("-")[1].strip()
                if new_user not in target_users:
                    target_users.append(new_user)
                    user_data[new_user] = {"answers": [], "question_index": 0, "processing": False}
                    await telegram_client.send_message(user, f"{new_user} başarıyla eklendi.")
                    await send_question(new_user)
                else:
                    await telegram_client.send_message(user, f"{new_user} zaten listede.")

            elif message.startswith("kullanicisil-"):
                remove_user = message.split("-")[1].strip()
                if remove_user in target_users:
                    ...
                    await telegram_client.send_message(user, f"{remove_user} başarıyla silindi.")
                else:
                    await telegram_client.send_message(user, f"{remove_user} listede bulunamadı.")

            else:
                # Normal kullanıcı mesajlarını işle
                await handle_response(event)

        @telegram_client.on(events.NewMessage(from_users=target_users))
        async def handle_response(event):
            """Kullanıcıdan gelen yanıtları işler."""
            user = event.sender_id
            if user not in user_data:
                user_data[user] = {"answers": [], "question_index": 0, "processing": False}

            # "Baştan başla" komutunu kontrol et
            if event.raw_text.strip().lower() == "baştan başla":
                user_data[user] = {"answers": [], "question_index": 0, "processing": False}
                await telegram_client.send_message(user, "Sorular baştan başlatılıyor.")
                await send_question(user)
                return

            # Kullanıcının işlem süreci devam ediyorsa
            if user_data[user]["processing"]:
                await telegram_client.send_message(user, "Cevabınız işleniyor. Lütfen bekleyiniz.")
                return

            answer = event.raw_text.strip()
            current_index = user_data[user]["question_index"]

            # Yanıtı kaydet ve sıradaki soruya geç
            if current_index < len(soracaklarim):
                user_data[user]["answers"].append(answer)
                user_data[user]["question_index"] += 1
                await send_question(user)
            else:
                # Tüm sorular tamamlandıysa, işlemi başlat
                user_data[user]["processing"] = True
                try:
                    result = await asyncio.to_thread(myanafonk, user_data[user]["answers"])
                    await telegram_client.send_message(user, result)
