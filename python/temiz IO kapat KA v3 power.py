...
def AnaFonkKA(raw_text):
    ...
    #print(result)
    if result[0][0]>-1 and io1d[-1]>50.1 and (extract_market_buying_power(mytextio[0])>1 or cift_ema_sinyal(iopower)[0]) and cift_ema_sinyal(io1d)[0]:#and cift_ema_sinyal(io1d)[0] and io1d[-1]!=49.9 and io1d[-1]!=50 and io1d[-1]!=50.1: # and not io15m1h4hdusuktemi():
        myFKAlist=[]
        kadakilonglar=[]
        for coin in result:
            print(coin[0], coin[2],coin[3],coin[5],coin[6])
            if binle(coin[0]) in mysymbols3 and coin[2]<1.7 and coin[2]>1.1 and coin[3]>1.005 and coin[5]==True and coin[6]<10 and acabilirmiyim(binle(coin[0])):
