...
def almakkosulu():
    myresult = (io1d[-1]>48.9 and apalayimmi and (cift_ema_sinyal(io1d)[0]) and (not son_bes_esit_mi(io1d))) or (io1d[-1]>50 and apalayimmi)
    return myresult
...
def AnaFonkKA(raw_text):
    ...
    if almakkosulu(): #result[0][0]>-1:# and io1d[-1]>50.1 and (extract_market_buying_power(mytextio[0])>1 or cift_ema_sinyal(iopower)[0]) and cift_ema_sinyal(io1d)[0]:#and cift_ema_sinyal(io1d)[0] and io1d[-1]!=49.9 and io1d[-1]!=50 and io1d[-1]!=50.1: # and not io15m1h4hdusuktemi():
        ...
        for coin in result:
            ...
            if binle(coin[0]) in mysymbols3 and coin[1]<1.6 and coin[1]>1 and coin[2]>1.01 and coin[3]<6.5: # and acabilirmiyim(binle(coin[0])):
...
def AnaFonkGriov2(raw_text):
    ...
    for mylist in parsed_data:
        if mylist[1]>5 and mylist[2]>1.5 and mylist[3]>1 and mylist[3]<1.65 and mylist[4][3]>50 and mylist[4][4]>50:
            print(mylist[0])
